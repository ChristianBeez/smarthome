...
while True:
    utc=pytz.UTC
    time_now = datetime.now(pytz.timezone('Europe/Berlin'))
    cfg = load_config()
    SchlafRollo = cfg.get('SchlafRollo', False)
    christmas   = cfg.get('christmas', False)
    extremeHot  = cfg.get('extremeHot', False)
